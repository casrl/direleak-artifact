...
def _sum_sc(repo):
    ...
    out.append("bright block-diagonal; confusions fall between closely related models")
    return out
...
def _sum_gram(repo):
    ...
    yi = d["y_inst"]
    return [f"{len(yi)} traces, {len(set(yi.tolist()))} model instances",
            f"{nl} samples (200 ms) x {nc} HitME sets, log miss counts"]
